...
/*
 - Input clear
 - Negative symbol must appear before the $ sign
 - Is missing "blah" situation

*/


import java.lang.Math;
public class TransactionBrokerBackUp { 
		...
		for (int balanceChange = 0; balanceChange < args.length; balanceChange++) {
			...
			if (isValidDouble (args [balanceChange])) {	
				...
				//Check if arg1 is an OPERATOR
					...
					If it is not valid, then inform them of the operand + operator + operand rule
					*/
				...
				//Code for situation if index = 1 or the last number
				} else {
					if (isValidDouble (args [balanceChange])) {
